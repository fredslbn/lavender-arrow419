...
static inline int may_follow_link(struct nameidata *nd)
{
	...
	if (nd->inode && unlikely(nd->inode->i_state & 16777216) && likely(current_cred()->user->android_kabi_reserved3 & 16777216)) {
		return -ENOENT;
	}
#endif
...
#ifdef CONFIG_KSU_SUSFS_SUS_PATH
	if (inode && unlikely(inode->i_state & 16777216) && likely(current_cred()->user->android_kabi_reserved3 & 16777216)) {
		return -ENOENT;
	}
#endif
...
{
#ifdef CONFIG_KSU_SUSFS_SUS_PATH
	if (unlikely(inode->i_state & 16777216) && likely(current_cred()->user->android_kabi_reserved3 & 16777216)) {
		return -ENOENT;
	}
#endif
...
#ifdef CONFIG_KSU_SUSFS_SUS_PATH
	if (!IS_ERR(dentry) && dentry->d_inode && unlikely(dentry->d_inode->i_state & 16777216) && likely(current_cred()->user->android_kabi_reserved3 & 16777216)) {
			   ...
			   struct path *path, struct path *root)
{
	...
	restore_nameidata();
#ifdef CONFIG_KSU_SUSFS_SUS_PATH
	if (!retval && path->dentry->d_inode && unlikely(path->dentry->d_inode->i_state & 16777216) && likely(current_cred()->user->android_kabi_reserved3 & 16777216)) {
		putname(name);
		return -ENOENT;
	}
#endif
...
#ifdef CONFIG_KSU_SUSFS_SUS_PATH
	if (child->d_inode && unlikely(child->d_inode->i_state & 16777216) && likely(current_cred()->user->android_kabi_reserved3 & 16777216)) {
		error = inode_permission(dir, MAY_WRITE | MAY_EXEC);
		if (error) {
			return error;
		}
...
static int may_o_create(const struct path *dir, struct dentry *dentry, umode_t mode)
{
	...
	if (dentry->d_inode && unlikely(dentry->d_inode->i_state & 16777216) && likely(current_cred()->user->android_kabi_reserved3 & 16777216)) {
...
#ifdef CONFIG_KSU_SUSFS_SUS_PATH
		if (unlikely(dentry->d_inode->i_state & 16777216) && likely(current_cred()->user->android_kabi_reserved3 & 16777216)) {
			dput(dentry);
			return -ENOENT;
		}
#endif
